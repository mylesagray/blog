...
<pre><code class="sh">govc folder.create /vSAN-DC/vm/k8s
...
<pre><code class="sh">sudo tee /etc/kubernetes/vsphere.conf &gt;/dev/null &lt;&lt;EOF
[Global]
...
<pre><code class="yaml">sudo tee /etc/kubernetes/kubeadminitmaster.yaml &gt;/dev/null &lt;&lt;EOF
