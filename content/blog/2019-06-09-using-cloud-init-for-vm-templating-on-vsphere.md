...
cover:
  image: /uploads/2019/06/Screenshot-2019-06-09-19.36.35.png
categories:
  - Automation
  - Cloud Frameworks
  - Featured
...
<pre><code class="json">$ cat ubuntu.json
{
...
<pre><code class="yaml">write_files:
  - content: |
        {
...
<pre><code class="sh">cloud-init devel schema --config-file my-user-data-file.txt
</code></pre>
...
<pre><code class="sh">base64 user-data | base64 -D # Note - on Linux it's lowercase -d
</code></pre>

If all is well, run it again without the decode and copy the `base64` encoded string it outputs:

<pre><code class="sh">base64 user-data
</code></pre>
...
<pre><code class="sh">Warning: Line 107: Unable to parse 'enableMPTSupport' for attribute 'key' on element 'Config'.
</code></pre>
...
<pre><code class="sh">until govc vm.info -json Ubuntu1804Template | jq -r '.VirtualMachines[].Runtime.PowerState' | grep -q  "poweredOff"; do sleep 5; done
...
<pre><code class="powershell">&gt; Connect-VIServer 10.198.16.4 -User administrator@vsphere.local -Password P@ssw0rd
...
<pre><code class="sh">govc vm.clone -vm Ubuntu1804Template -customization=Ubuntu customisation-test
</code></pre>
...
<pre><code class="sh">govc vm.clone -vm Ubuntu1804Template -customization=Ubuntu k8s-master01
...
<pre><code class="sh">govc folder.create /DC01/vm/k8s
...
<pre><code class="sh">ssh ubuntu@10.198.25.84
</code></pre>
...
<pre><code class="sh">$ kubectl get nodes -o wide
...
<pre><code class="sh">scp ubuntu@10.198.25.84:~/discovery.yaml discovery.yaml
</code></pre>

Then upload it to the worker nodes.

<pre><code class="sh">scp discovery.yaml ubuntu@10.198.25.92:~/discovery.yaml
...
<pre><code class="sh">ubuntu@k8s-master01:~$ kubectl get nodes -o wide
NAME           STATUS   ROLES    AGE   VERSION   INTERNAL-IP    EXTERNAL-IP    OS-IMAGE             KERNEL-VERSION      CONTAINER-RUNTIME
...
<pre><code class="sh">ubuntu@k8s-master01:~$ kubectl describe nodes | grep "ProviderID"
...
<pre><code class="sh">govc device.cdrom.eject -vm  k8s-master01
...
<pre><code class="sh">govc vm.question -vm k8s-master01 -answer 0
...
Why not follow [@mylesagray on Twitter][25] for more like this!
 ...
 [22]: https://dl.dropboxusercontent.com/s/c2s7iq6vqehzcvm/Screenshot%202019-06-09%2019.36.35.png
